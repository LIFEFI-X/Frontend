...
export const allocationChecker = async (
  ...
  try {
    ...
    if (mintCounter) {
      ...
      message.error('Allocation Guard not Initialized!')
...
export const solBalanceChecker = (solBalance: SolAmount, solAmount: SolAmount) => {
...
export const tokenBalanceChecker = async (umi: Umi, tokenAmount: bigint, tokenMint: PublicKey) => {
...
export const checkDateRequired = (guards: { label: string; guards: GuardSet }[]) => {
...
export const checkSolBalanceRequired = (guards: { label: string; guards: GuardSet }[]) => {
...
export const checkTokensRequired = (guards: { label: string; guards: GuardSet }[]) => {
  let nftBalanceRequired: boolean = false
  guards.forEach((guard) => {
    if (guard.guards.nftBurn || guard.guards.nftGate || guard.guards.nftPayment) {
...
export const calculateMintable = (mintableAmount: number, newAmount: number) => {
  ...
  if (!import.meta.env.VITE_APP_MICROLAMPORTS) return mintableAmount
  let maxmintamount = 0
  try {
    maxmintamount = Number(import.meta.env.VITE_APP_MICROLAMPORTS)
  } catch (e) {
    console.error('import.meta.env.VITE_APP_MICROLAMPORTS is not a number!', e)
