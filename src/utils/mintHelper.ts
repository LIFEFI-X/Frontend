...
import { type DigitalAssetWithToken, TokenStandard } from '@metaplex-foundation/mpl-token-metadata'
import {
  some,
  type Umi,
  transactionBuilder,
...
import { type GuardReturn } from './checkerHelper'
...
export const chooseGuardToUse = (guard: GuardReturn, candyGuard: CandyGuard) => {
  const guardGroup = candyGuard?.groups.find((item) => item.label === guard.label)
  if (guardGroup) {
    return guardGroup
  }
